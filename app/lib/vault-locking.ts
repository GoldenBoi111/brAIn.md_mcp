...
export type LockOperation = { action: "lock" | "unlock" | "toggle"; path: string; reason?: string };

export type LockAuditEntry = { timestamp: number; action: "lock" | "unlock"; path: string; reason?: string; changedBy?: string };
...
export async function lockPathsWithAudit(vaultRoot: string, paths: string[], reason?: string): Promise<LockAuditEntry[]> {
	const audits: LockAuditEntry[] = [];
	for (const path of paths) {
		if (await lockPath(vaultRoot, path)) {
			audits.push({ timestamp: Date.now(), action: "lock", path, reason });
		}
	}
	return audits;
}

export async function unlockPathsWithAudit(vaultRoot: string, paths: string[], reason?: string): Promise<LockAuditEntry[]> {
	const audits: LockAuditEntry[] = [];
	for (const path of paths) {
		if (await unlockPath(vaultRoot, path)) {
			audits.push({ timestamp: Date.now(), action: "unlock", path, reason });
		}
	}
	return audits;
}

export async function filterByLockStatus(vaultRoot: string, paths: string[], locked: boolean): Promise<string[]> {
	const locks = await loadLockSet(vaultRoot);
...
export async function performLockOperations(vaultRoot: string, operations: LockOperation[]): Promise<LockAuditEntry[]> {
	const locks = await loadLockSet(vaultRoot);
	const audits: LockAuditEntry[] = [];
	...
	for (const op of operations) {
		...
		if (changed) {
			audits.push({ timestamp: Date.now(), action: op.action === "toggle" ? (locks.has(op.path) ? "lock" : "unlock") : op.action, path: op.path, reason: op.reason });
		}
	}
