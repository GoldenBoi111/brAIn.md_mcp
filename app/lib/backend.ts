...
async function embedTexts(texts: string[]): Promise<number[][]> {
	const response = await fetch(`${DEFAULT_JASPER_URL}/embed`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ texts }) });
	if (!response.ok) {
...
async function qdrantRequest(method: string, endpoint: string, payload?: Record<string, unknown>): Promise<any> {
	const response = await fetch(`${DEFAULT_QDRANT_URL}${endpoint}`, { method, headers: { "Content-Type": "application/json" }, body: payload ? JSON.stringify(payload) : undefined });
	if (!response.ok) {
...
async function ensureCollection(): Promise<void> {
	try {
		await qdrantRequest("PUT", `/collections/${DEFAULT_COLLECTION}`, { vectors: { size: DEFAULT_VECTOR_SIZE, distance: "Cosine" } });
	} catch (error: any) {
...
export async function qdrantUpdateFile(options: { tenantId: string; fileId: string; content: string; embeddingModel: string; legacyFilePath?: string }): Promise<{ created_or_updated_chunks: number; deleted_chunks: number; existing_chunks: number }> {
	...
	for (let index = 0; index < chunks.length; index++) {
		...
		newIds.add(pointId);
		newPoints.push({ id: pointId, vector, payload: { tenant_id: tenantId, file_id: fileId, chunk_index: index, embedding_model: embeddingModel, chunk_hash: chunkHash } });
	}
	const pointsToDelete = Array.from(existingIds).filter((id) => !newIds.has(id));
	if (pointsToDelete.length) {
		await qdrantRequest("POST", `/collections/${DEFAULT_COLLECTION}/points/delete`, { points: pointsToDelete, wait: true });
	}
	if (newPoints.length) {
		await qdrantRequest("PUT", `/collections/${DEFAULT_COLLECTION}/points`, { points: newPoints, wait: true });
	}
	return { created_or_updated_chunks: newPoints.length, deleted_chunks: pointsToDelete.length, existing_chunks: existing.length };
}
...
export async function embedPayload(texts: string[]): Promise<{ model_name: string; dimension: number; vectors: number[][] }> {
	...
	return { model_name: String(data.model_name ?? ""), dimension: Number(data.dimension ?? DEFAULT_VECTOR_SIZE), vectors: data.vectors };
}
